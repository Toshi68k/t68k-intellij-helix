...
| `space + F` | `file_picker_in_current_directory` | — | Open fuzzy file picker in current buffer's directory |
...
- `Alt+n` / `Alt+Right` &rarr; Select next sibling AST element (`select_next_sibling`)
- `Alt+b` &rarr; Move to start of parent AST node (`move_parent_node_start`)
- `Alt+e` &rarr; Move to end of parent AST node (`move_parent_node_end`)
...
- `:file-picker-in-current-directory` &rarr; Open fuzzy file picker in current buffer's directory
