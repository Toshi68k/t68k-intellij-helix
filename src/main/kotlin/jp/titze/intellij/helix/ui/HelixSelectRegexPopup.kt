...
object HelixSelectRegexPopup {
    ...
    fun show(editor: Editor, isSplit: Boolean = false) {
        ...
        val statusLabel = JBLabel("Enter: apply | Esc: cancel")
        ...
        fun updateStatus() {
            ...
            if (query.isEmpty()) {
                statusLabel.text = "Enter: apply | Esc: cancel"
                statusLabel.foreground = UIUtil.getContextHelpForeground()
            } else {
                val matches = HelixActions.countRegexMatches(editor, query)
                statusLabel.text = "$matches match${if (matches == 1) "" else "es"} (Enter: apply | Esc: cancel)"
